...
import { Bell, Bookmark, FileDown, Home, LayoutDashboard, MessageSquare, Palette, UserRoundCog, Video } from "lucide-react"
...
const MobileSidebar = ({ show, onToggleMenu, onToggleTheme }) => {
    ...
    const [openSettingCustomBorderAvatar, setOpenSettingCustomBorderAvatar] = useState(false);
    ...
    return (
        ...
        <Offcanvas show={show} onHide={onToggleMenu} className="mobile-sidebar">
            ...
            <Offcanvas.Body>
                <ListGroup>
                    ...
                    <ListGroup.Item className="py-4 border-0 shadow-sm d-flex align-items-center gap-4 mobile-sidebar-item" onClick={handleExportData}>
                        <FileDown className="sidebar-icon" size={20} />
                        <h3 className="mb-0">Download your data</h3>
                    </ListGroup.Item>
                    <ListGroup.Item className="py-4 border-0 shadow-sm d-flex align-items-center gap-4 mobile-sidebar-item" onClick={onOpenSettingProfile}>
                        <UserRoundCog className="sidebar-icon" size={20} />
                        <h3 className="mb-0">Profile Settings</h3>
                    </ListGroup.Item>
                    <ListGroup.Item className="py-4 border-0 shadow-sm d-flex align-items-center gap-4 mobile-sidebar-item" onClick={onOpenSettingCustomBorderAvatar}>
                        <Palette className="sidebar-icon" size={20} />
        ...
        <CustomBorderAvatarSetting show={openSettingCustomBorderAvatar} onHide={onOpenSettingCustomBorderAvatar} />
